...
func _process(_delta):
	...
	if Engine.is_editor_hint():
		icon_sprite.texture = item_texture
	# Add item to the inventory if the Player presses "E" withing range
	if player_in_range == true and Input.is_action_just_pressed("ui_add"):
		pickup_item()
...
# Function to set the data to item_data to be used to drop the item
